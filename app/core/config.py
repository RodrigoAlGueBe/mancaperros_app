...
class Settings(BaseSettings):
    ...
    refresh_token_expire_days: int = Field(
        default=7,
        ge=1,
        le=30,  # SECURITY: Max 30 days for refresh tokens
        description="JWT refresh token expiration time in days (recommended: 7)"
    )
    ...
    def validate_secret_key(cls, v: str, info) -> str:
        ...
        1. SECRET_KEY must be explicitly set in production (never use default)
        2. Minimum length of 32 characters for cryptographic security
        3. Fails securely with clear error message if validation fails
        """
        if v == "CHANGE_ME_IN_PRODUCTION_USE_SECURE_RANDOM_KEY":
            # Check the environment from the data being validated
            # This handles both env variables and .env file correctly
