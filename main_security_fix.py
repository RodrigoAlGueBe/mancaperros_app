...
# # TODO condicionar este middleware para que solo se aplique en produccion
# @app.middleware("http")
...
# ****************************************************** POST ******************************************************
# SECURITY: Endpoint publico - necesario para registro de nuevos usuarios
# No requiere autenticacion por naturaleza del flujo de registro
@app.post("/users/")
...
async def end_routine(current_user: Annotated[models.User, Depends(get_current_user)], exercises_summary:dict, db: Session = Depends(get_db)):
    ...
    ## Comprobamos que el numero de ejercicios por rutina coincida
    ...
    for exercise_db in last_routine.exercises:
        ...
        # Actualizamos el numero del ejercicio
...
# ****************************************************** GET *******************************************************
# ============================================================================
# SECURITY DOCUMENTATION - CLASIFICACION DE ENDPOINTS
# ============================================================================
# ENDPOINTS PUBLICOS (sin autenticacion):
#   - POST /users/           : Registro de usuarios (necesario para onboarding)
#   - POST /token            : Login y obtencion de token JWT
#   - GET /test/             : Health check basico (no expone datos sensibles)
#
# ENDPOINTS PROTEGIDOS (requieren JWT):
#   - Todos los demas endpoints requieren autenticacion via Depends(get_current_user)
#   - Algunos endpoints tienen validacion adicional de autorizacion (RBAC/ownership)
# ============================================================================
...
# SECURITY: Endpoint protegido - requiere autenticacion JWT
# Solo permite consultar el propio email del usuario autenticado para prevenir enumeracion de usuarios
# Riesgo mitigado: User enumeration attack, exposicion de datos personales (OWASP A01:2021)
...
def get_user_by_email(
    ...
    # SECURITY: Validacion de autorizacion - usuario solo puede consultar su propio email
    # Esto previene enumeracion de usuarios y exposicion de datos de terceros
    if current_user.username != user_email:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="No autorizado para consultar datos de otros usuarios"
        )
...
# SECURITY: Endpoint protegido - requiere autenticacion JWT
# SECURITY WARNING: Este endpoint expone la lista completa de usuarios.
# TODO: Evaluar si este endpoint es realmente necesario para la aplicacion.
# Si no es necesario, considerar eliminarlo completamente.
# Si es necesario solo para administradores, implementar control de acceso basado en roles (RBAC).
# Riesgo mitigado: Exposicion publica de lista de usuarios (OWASP A01:2021 - Broken Access Control)
@app.get("/get_users/")
def get_all_users(
    current_user: Annotated[models.User, Depends(get_current_user)],
    db: Session = Depends(get_db)
...
# SECURITY: Endpoint protegido - requiere autenticacion JWT
# Este endpoint expone informacion de rutinas por ID de plan de ejercicios
# Riesgo mitigado: Acceso no autorizado a datos de planes de ejercicio (OWASP A01:2021)
@app.get("/exercise_plans/{exercise_plan_id}/rutines")
async def get_all_rutines_for_exercise_plan(
...
# SECURITY: Endpoint publico - Health check basico
# No expone datos sensibles, solo confirma que el servicio esta activo
# Util para monitoring y load balancers
@app.get("/test/")
def get_test():
...
def get_muscular_groups_for_exercise_plans(exercise_plan_name, current_user: Annotated[models.User, Depends(get_current_user)], db: Session = Depends(get_db)):
    # TODO, refactorizar esta funcion para eliminar exercise_plan_name, y tambien en el front
...
# ***************************************************** LOGIN ******************************************************
# SECURITY: Endpoint publico - necesario para autenticacion
# No requiere autenticacion previa por naturaleza del flujo de login
